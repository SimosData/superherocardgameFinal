document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- Lives Configuration ---
    const LIVES_ON_RESTART = 5; // <<< NEW: Specific number of lives on restart
    const MIN_LIVES_MANUAL = 0; // <<< NEW: Minimum lives allowed via manual buttons (can be 0)
    // const DEFAULT_STARTING_LIVES = 3; // <<< Replaced by LIVES_ON_RESTART for initialization
    // const MIN_LIVES = 1; // <<< Replaced by MIN_LIVES_MANUAL
    ...
    const MIN_CARD_SIZE_MULTIPLIER = 0.5; // Minimum size (e.g., 60%)
    const MAX_CARD_SIZE_MULTIPLIER = 4.0; // Maximum size (e.g., 150%)
    const CARD_SIZE_STEP = 0.1;           // How much to change size per click

    // --- Pinch Zoom ---
    const MIN_PAGE_ZOOM_SCALE = 0.5; // Min zoom level for the page
    const MAX_PAGE_ZOOM_SCALE = 4.0; // Max zoom level for the page
    ...
    let matchesFound = 0;
    let currentScore = 0;
    let currentLives = LIVES_ON_RESTART; // <<< Initialize with restart value
    ...
    function updateLivesDisplay() {
        ...
        // <<< MODIFIED: Use MIN_LIVES_MANUAL for the down button check
        if (btnLivesDown) btnLivesDown.disabled = (currentLives <= MIN_LIVES_MANUAL || gameIsActive || isPaused);
    ...
    function toggleLifeButtons(enable) {
        ...
        // <<< MODIFIED: Use MIN_LIVES_MANUAL for the down button check
        if (btnLivesDown) btnLivesDown.disabled = !enable || disableReason || currentLives <= MIN_LIVES_MANUAL;
    ...
    function updatePausePlayButton() {
        ...
        // <<< MODIFIED: Check currentLives > 0 (game over condition)
    ...
    function handleMismatch() {
        ...
        currentLives--;
        updateLivesDisplay(); // <<< Update display AFTER decrementing

        // <<< MODIFIED: Check if lives are now 0 or less
        if (currentLives <= 0) {
            handleGameOver('lives'); // Pass reason
        } else {
            // Unflip cards after a delay only if game is not over
    ...
    function handleWin() {
        ...
        setTimeout(() => {
            ...
            // <<< MODIFIED: Call restartGame instead of initializeGame directly
            // This ensures the restart button logic (like setting lives) is used
            setTimeout(restartGame, WIN_DISPLAY_DELAY);
    ...
    function handleGameOver(reason) {
        ...
        setTimeout(() => {
            ...
            // <<< MODIFIED: Call restartGame instead of initializeGame directly
            // This ensures the restart button logic (like setting lives) is used
            setTimeout(restartGame, GAME_OVER_DISPLAY_DELAY);
    ...
    function setDifficulty(difficulty) {
        ...
        if (currentDifficulty !== difficulty && difficultySettings[difficulty]) {
            ...
            // <<< MODIFIED: Call restartGame to ensure lives reset correctly
            restartGame(); // Re-initialize with new difficulty and reset lives
    ...
    // <<< NEW Function: Handles restart logic including setting lives >>>
    function restartGame() {
        console.log("--- Restarting Game ---");
        currentLives = LIVES_ON_RESTART; // <<< Set lives to the defined restart value
        initializeGame(); // Call the main initialization logic
    }

    function initializeGame() {
        // <<< NOTE: This function now focuses ONLY on resetting game elements,
        //     lives are set in restartGame() which calls this function. >>>
        console.log(`--- Initializing Game Board: ${currentDifficulty} ---`);
        ...
        // --- Reset Game State (excluding lives, handled by restartGame) ---
        matchesFound = 0;
        currentScore = 0;
        // currentLives = LIVES_ON_RESTART; // <<< MOVED to restartGame()
        gameIsActive = false;
        ...
        // --- Update UI Elements ---
        updateScoreDisplay();
        updateLivesDisplay(); // <<< Update display with lives set by restartGame()
        ...
        console.log(`Game Board Initialized: ${settings.rows}x${settings.cols} (${requiredPairs * 2} cards)`);
    }
    ...
    difficultyButtons.forEach(button => {
        if (button && button.dataset.difficulty) {
            // <<< MODIFIED: Listener now calls setDifficulty directly
    ...
    if (btnLivesDown) {
        btnLivesDown.addEventListener('click', () => {
            // <<< MODIFIED: Check against MIN_LIVES_MANUAL
            if (!gameIsActive && !isPaused && currentLives > MIN_LIVES_MANUAL) {
                triggerVibration(VIBRATE_BUTTON_PRESS);
                currentLives--;
                updateLivesDisplay();
            }
    ...
    if (btnRestart) {
        btnRestart.addEventListener('click', () => {
            ...
            // <<< MODIFIED: Call the new restartGame function >>>
            restartGame();
    ...
    if (btnPausePlay) {
        btnPausePlay.addEventListener('click', () => {
            // <<< MODIFIED: Check currentLives > 0 before allowing pause/play
            if (!gameIsActive || currentLives <= 0) return; // Don't allow pause if game hasn't started or is over
            triggerVibration(VIBRATE_BUTTON_PRESS);
            isPaused = !isPaused;
            if (isPaused) {
                ...
                // Only unlock if game is still active (might have ended while paused?)
                lockBoard = !(gameIsActive && currentLives > 0);
                startBackgroundRotation();
                startCountdown(); // Resumes timer
                startRealTimeClock();
                // Re-enable based on game state (should still be disabled if game active)
                toggleLifeButtons(!gameIsActive);
                toggleTimerAdjustButtons(!gameIsActive);
            }
    ...
    // <<< MODIFIED: Call restartGame() on initial load to set lives correctly >>>
    restartGame(); // Start the game with default settings and 5 lives
